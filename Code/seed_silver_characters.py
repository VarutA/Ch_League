"""
Create/reset Silver roster and generate LeaguePIC_1..50 for each Silver team.

Targets:
- Character/Character Silver
- Character/Pic Selection/<silver team>

This only clears Character Silver and matching Pic Selection folders for old Silver names.
"""
...
MASTER = os.path.join(CHARACTER_ROOT, "Character Silver")
PICS = os.path.join(CHARACTER_ROOT, "Pic Selection")

IMG_SIZE = 72
NUM_PICS = 50
DEFAULT_NUM_CHARS = 100
...
def _list_silver_names(base: str) -> list[str]:
...
def _clear_old_silver_entries(keep_templates: bool) -> None:
    os.makedirs(MASTER, exist_ok=True)
    os.makedirs(PICS, exist_ok=True)

    old_silver_names = _list_silver_names(MASTER)
    for name in old_silver_names:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=f"Number of Silver characters to generate (default: {DEFAULT_NUM_CHARS})",
    )
    parser.add_argument(
        "--keep-templates",
        action="store_true",
        help="Keep folders whose names start with ${ in Character Silver.",
    )
    ...
    _clear_old_silver_entries(keep_templates=args.keep_templates)

    names = _unique_names(args.count)
    for name in names:
    ...
    print(f"Created {args.count} Silver characters under:\n  {MASTER}\n  {PICS}")
    for name in names:
        print(f"  - {name}")
